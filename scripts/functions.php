...
function getkind($filename, $cat)
{
   if (strpos($filename, "archive"))		// archive
      return("archive");
   elseif (strpos($filename, "aboutme"))	// cv
      return("aboutme");
...
function content ($type, $cat, $filename)
{
   ...
   elseif ($type == 'aboutme')
   {
      echo '      
                  <p>
                  I\'m a senior in electrical engineering at the University of Michigan.  I spend my class time
                  studying computer architecture and my free time developing things like this blog engine.
                  Feel free to contact me at mcjustin at umich dot edu.
                  </p>
                  ' . "\n";
   }
   elseif ($type == 'links')
   ...
   elseif ($stage == 3)																																			// Update the database
   {
      ...
      $rows = mysql_num_rows($QUERY);
		...
		else
		{
         ...
         {     
            $sql = "INSERT INTO `Entries` (`Index`, `Number`, `Address`, `Title`, `Date`, `Updated`, `Category`, `Entry`) VALUES ('$rows', '$number', '$address', '$title', '$date', '$date', '$cat', '$html');";
...
// atom and rss ain't working since I lost both of the original files!  The functions need something to work with.      
//      atom($cat);
//      rss($cat);
